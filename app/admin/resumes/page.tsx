...
export default function AdminResumesPage() {
  ...
  if (loading) return <div className="flex min-h-screen items-center justify-center bg-gray-950 text-white">Loading...</div>;
        ...
        <main className="p-6">
          <div className="mb-6 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-400">{total} resume{total !== 1 ? "s" : ""} in the system</p>
            </div>
          </div>
          ...
          <div className="mb-6 flex gap-3">
            ...
            <Button onClick={() => { setPage(1); fetchResumes(); }} variant="outline" className="border-gray-700">
              Search
            </Button>
          </div>
          ...
          {selectedResume && (
            <div className="mb-8">
              <Button onClick={closeDetail} variant="ghost" className="mb-4 text-gray-400">
                ...
                <CardContent className="p-6">
                  <div className="mb-6 flex items-start justify-between">
                    <div>
                      ...
                      <p className="text-sm text-gray-400">{selectedResume.profile?.email}</p>
                      {selectedResume.resume?.target_role && (
                        <Badge variant="outline" className="mt-2 border-violet-500/30 text-violet-400">
                          {selectedResume.resume.target_role}
                        </Badge>
                      )}
                    </div>
                  ...
                  {selectedResume.resume?.resume_data && (() => {
                    ...
                    <div className="space-y-4">
                      ...
                      {typeof rd.summary === 'string' && rd.summary && (
                        <div>
                          <h3 className="mb-1 text-sm font-bold text-gray-300">Professional Summary</h3>
                          <p className="text-sm text-gray-400">{String(rd.summary)}</p>
                        </div>
                      )}
                      ...
                      {Array.isArray(rd.skills) && (
                        <div>
                          <h3 className="mb-2 text-sm font-bold text-gray-300">Skills</h3>
                          <div className="flex flex-wrap gap-1.5">
                            {(rd.skills as string[]).map((skill, i) => (
                              <Badge key={i} variant="secondary" className="bg-gray-800 text-gray-300">{skill}</Badge>
                            ))}
                          </div>
                        </div>
                      )}
                      ...
                      {Array.isArray(rd.experience) && (
                        <div>
                          <h3 className="mb-2 text-sm font-bold text-gray-300">Experience</h3>
                          {(rd.experience as Array<Record<string, string>>)
                            .filter(e => e.position)
                            .map((exp, i) => (
                              <div key={i} className="mb-2 rounded-lg bg-gray-800/50 p-3">
                                <p className="font-semibold text-white">{exp.position}</p>
                                <p className="text-sm text-violet-400">{exp.company}{exp.location ? ` — ${exp.location}` : ""}</p>
                      ...
                      {Array.isArray(rd.education) && (
                        <div>
                          <h3 className="mb-2 text-sm font-bold text-gray-300">Education</h3>
                  ...
                  {selectedResume.files.length > 0 && (
                    <div className="mt-6 border-t border-gray-800 pt-4">
                      <h3 className="mb-3 text-sm font-bold text-gray-300">Exported Files</h3>
                      <div className="space-y-2">
                        {selectedResume.files.map(file => (
                          <div key={file.id} className="flex items-center justify-between rounded-lg bg-gray-800/50 px-4 py-2">
                            <div className="flex items-center gap-3">
                              <FileText className="h-4 w-4 text-gray-400" />
                              <div>
                                <p className="text-sm text-white">{file.file_name}</p>
                                <p className="text-xs text-gray-500">{new Date(file.created_at).toLocaleDateString()} • {file.file_type.toUpperCase()}</p>
                              </div>
                            </div>
                            {file.file_url && file.file_url !== "local_export" && (
                              <a href={file.file_url} target="_blank" rel="noopener noreferrer" className="text-violet-400 hover:text-violet-300">
          ...
          {!selectedResume && (
            <>
              <div className="space-y-3">
                {fetching && <p className="text-center text-gray-400 py-8">Loading resumes...</p>}
                ...
                {resumes.map(r => (
                  <Card key={r.id} className="border-gray-800 bg-gray-900 transition-colors hover:border-gray-700">
                    <CardContent className="flex items-center justify-between p-4">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          {r.target_role && (
                            <p className="text-xs text-violet-400">{r.target_role}</p>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-4">
                        <div className="hidden text-right sm:block">
                          ...
                          <p className="text-[10px] text-gray-600">{r.template} • {r.color_theme}</p>
                        </div>
                        ...
                        <Button size="sm" variant="ghost" onClick={() => viewResume(r.user_id)} className="text-violet-400 hover:text-violet-300">
              ...
              {totalPages > 1 && (
                <div className="mt-6 flex items-center justify-center gap-3">
                  <Button variant="outline" size="sm" disabled={page <= 1} onClick={() => setPage(p => p - 1)} className="border-gray-700">
                    <ChevronLeft className="h-4 w-4" />
                  </Button>
                  <span className="text-sm text-gray-400">Page {page} of {totalPages}</span>
                  <Button variant="outline" size="sm" disabled={page >= totalPages} onClick={() => setPage(p => p + 1)} className="border-gray-700">
